...
def checkPair(array):
    ...
    col_index = random.randint(0, cols - 1)  # -2 zeby istnial kolejny element
    # print(col_index)
    if array[row_index][col_index] == array[row_index][(col_index - 1) % cols] or array[row_index][col_index] == \
            array[row_index][(col_index + 1) % cols]:
        return row_index, col_index
    else:
        return False


def changeOpinion(array, row_index, col_index):
    print()
    # todo zmiana pobliskich przy zgodzie


def runSim(probability, cycleCount):
    print()
    # todo
